   ...
   $(document).ready(function() {
	...
	<body>
		...
		$result = mysql_query("SELECT * FROM l1 WHERE name_last LIKE '$name_last%' AND cause_of_death LIKE '%$cause_of_death%' AND name_first LIKE '$name_first%' AND ( year(date_on_ledger) LIKE '%$year%' OR year(date_on_ledger) BETWEEN '%$startyear%' AND '%$endyear%' ) AND interment_at LIKE '%$interment_at%' AND occupation LIKE '%$occupation%' ORDER BY age_years ASC ");
